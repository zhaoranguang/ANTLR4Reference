...
public class Calc {
    public static void main(String[] args) throws Exception {
        ...
        int line = 1;                       // track input exprline numbers
        ...
        while (expr != null) {              // while we have more expressions
            // create new lexer and token stream fo each line (expression)
            ANTLRInputStream input = new ANTLRInputStream(expr+"\n");
            ExprLexer lexer = new ExprLexer(input);
            lexer.setLine(line);            // notify lexer of input position
            lexer.setCharPositionInLine(0);
            CommonTokenStream tokens = new CommonTokenStream(lexer);
            parser.setInputStream(tokens);  // notify lexer of input position
